...
DATE_PREFIX_RE = re.compile(
    rf"^\s*(?:\d{{1,2}}\s+(?:{MESES})\.?)\s+",
    flags=re.I
)


def aplicar_fuente_run(run, fuente, tamano, bold=False):
...
def corregir_acentos(texto: str) -> str:
    ...
    reemplazos = {
        ...
        "a´": "á", "e´": "é", "i´": "í", "o´": "ó", "u´": "ú",
        "˜n": "ñ", "˜N": "Ñ",
        "Dise˜nar": "Diseñar", "dise˜nar": "diseñar",
        "dise˜no": "diseño", "disen˜o": "diseño",
        "desempe˜no": "desempeño",
        "acompa˜na": "acompaña",
        "tama˜no": "tamaño",
        "tama˜nos": "tamaños",
        "peque˜nos": "pequeños",
        "ense˜nanza": "enseñanza",
        ...
        "Metodolog´ıa": "Metodología",
        "Bibliograf´ia": "Bibliografía",
        "Bibliograf´ıa": "Bibliografía",
        ...
        "l´ınea": "línea",
        "l´ıneas": "líneas",
        "estad´ıstica": "estadística",
        "estad´ısticos": "estadísticos",
        "estad´ısticas": "estadísticas",
        "emp´ırica": "empírica",
        "cr´ıtico": "crítico",
        "cr´ıtica": "crítica",
        "t´ecnico": "técnico",
        ...
        "t´ecnicas": "técnicas",
        "t´ecnicos": "técnicos",
        "diagn´osti": "diagnósti",
        "din´amica": "dinámica",
        "din´amicas": "dinámicas",
        "p´erdida": "pérdida",
        "p´erdidas": "pérdidas",
        "b´asica": "básica",
        "b´asico": "básico",
        ...
        "S´ıntesis": "Síntesis",
        "s´ıntesis": "síntesis",
        "caracter´ısticas": "características",
        "caracter´ıstica": "característica",
        "anomal´ıas": "anomalías",
        "intuici´on": "intuición",
        "visi´on": "visión",
        "pr´actica": "práctica",
        "pr´acticas": "prácticas",
        "pr´actico": "práctico",
        "pr´acticos": "prácticos",
        "arquitect´onicas": "arquitectónicas",
        "hiperpar´ametros": "hiperparámetros",
        "transformaci´on": "transformación",
        "regularizaci´on": "regularización",
        "optimizaci´on": "optimización",
        "activaci´on": "activación",
        "reconstrucci´on": "reconstrucción",
        "validaci´on": "validación",
        "atenci´on": "atención",
        "representaci´on": "representación",
        "explosi´on": "explosión",
        "resoluci´on": "resolución",
        "comparaci´on": "comparación",
        "aplicaci´on": "aplicación",
        "tokenizaci´on": "tokenización",
        "clasificaci´on": "clasificación",
        "regresi´on": "regresión",
        "perdida": "pérdida",
    }
...
def detectar_encabezados(texto):
    ...
    for item in sorted(encontrados, key=lambda x: x[0]):
        if not depurados or not (
            depurados[-1][2] == item[2]
            and abs(depurados[-1][0] - item[0]) <= 2
        ):
...
def extraer_info_general(bloque_info):
    etiquetas = [
        ...
        "HORAS DE TRABAJO", "AUTÓNOMO", "AUTONOMO",
        "PROFESOR", "PROFESORES", "CORREO"
    ]
    ...
    info = {
        ...
        "area": extraer_valor_campo(bloque_info, "ÁREA", etiquetas)
        or extraer_valor_campo(bloque_info, "AREA", etiquetas),
        "titulo": extraer_valor_campo(bloque_info, "ASIGNATURA", etiquetas),
        "creditos": extraer_valor_campo(bloque_info, "CRÉDITOS", etiquetas)
        or extraer_valor_campo(bloque_info, "CREDITOS", etiquetas),
...
def empieza_vineta(linea):
    ...
    verbos = (
        "COMPRENDER", "ANALIZAR", "DISEÑAR", "INTERPRETAR",
        "APLICAR", "COMUNICAR", "ELABORAR", "EVALUAR",
        "FORMULAR", "SUSTENTAR", "PRESENTAR", "DOCUMENTAR",
        "IDENTIFICAR", "UTILIZAR", "CONSTRUIR", "EXPLICAR",
...
def reflow_bloque(texto, activar_vinetas=True):
    ...
    for l in lineas:
        ...
        if activar_vinetas and (
            l.startswith(("•", "-", "", "▪", "■"))
            or (contexto_vinetas and empieza_vineta(l))
        ):
            flush()
        ...
        if (
            n.endswith("IMPLICA")
            or n.endswith("INCLUYE")
            or norm(actual).endswith("IMPLICA")
            or norm(actual).endswith("INCLUYE")
        ):
            contexto_vinetas = True
...
def leer_matriz_alineacion(matriz_path, asignatura):
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        activo = (
            row[idx_activo]
            if idx_activo is not None and idx_activo < len(row)
            else "Sí"
        )
...
def eliminar_notas_al_pie(doc):
    """
    Elimina las referencias a notas al pie de la plantilla institucional.
    Esto quita los superíndices y evita que aparezcan los textos explicativos
    asociados a la fecha de actualización y tipología del curso.
    """

    # Elimina referencias a notas al pie en el cuerpo del documento.
    for ref in doc.element.xpath(".//w:footnoteReference"):
        parent = ref.getparent()
        if parent is not None:
            parent.remove(ref)

    # Elimina referencias a notas al pie dentro de tablas, encabezados o estructuras internas.
    for ref in doc.element.xpath(".//w:footnoteRef"):
        parent = ref.getparent()
        if parent is not None:
            parent.remove(ref)

    # Intenta eliminar la relación con footnotes.xml.
    try:
        rels_to_delete = []
...
def extraer_tabla_tematicas_por_texto(bloque_tematicas):
    lines = [
        limpiar_texto(l)
        for l in (bloque_tematicas or "").splitlines()
        if limpiar_texto(l)
    ]
        ...
        if re.fullmatch(r"\d{1,2}", l):
            ...
            if i + 1 < len(lines) and DATE_PREFIX_RE.match(lines[i + 1]):
                ...
                while j < len(lines):
                    ...
                    if (
                        re.fullmatch(r"\d{1,2}", lines[j])
                        or DATE_PREFIX_RE.match(lines[j])
                        or any(
                            x in nj
                            for x in [
                                "FESTIVO",
    ...
    with pdfplumber.open(path_pdf) as pdf:
        for page in pdf.pages:
            ...
            for tabla in tablas or []:
                ...
                rows = [
                    [limpiar_celda_texto(c) for c in row]
                    for row in tabla
                    if row and any(row)
                ]
                ...
                if not all(
                    x in norm(texto)
                    for x in ["SEMANA", "SESION", "CONTENIDO"]
                ):
                    continue
                ...
                for i, r in enumerate(rows[:4]):
                    ...
                    if (
                        any("SEMANA" in c for c in nr)
                        and any("SESION" in c for c in nr)
                        and any("CONTENIDO" in c for c in nr)
                    ):
                        header_idx = i
                        break
                ...
                for r in rows[header_idx + 1:]:
                    ...
                    con = quitar_fecha_inicio(
                        r[i_con] if i_con < len(r) else ""
                    )
...
def procesar_pdf(template_path, pdf_path, output_dir, matriz_path=None):
    ...
    datos_matriz = (
        leer_matriz_alineacion(matriz_path, info.get("titulo", ""))
        if matriz_path
        else {
            "competencias_programa": [],
            "rdap_programa": [],
    ...
    limpiar_e_insertar_en_placeholder(
        ...
        reflow_bloque(
            bloques.get("resultados", ""),
            activar_vinetas=True
        )
    )
    ...
    limpiar_e_insertar_en_placeholder(
        ...
        reflow_bloque(
            presentacion,
            activar_vinetas=False
        )
    )

    tabla_tematicas = extraer_tabla_tematicas_pdf(
        pdf_path,
        bloques.get("tematicas", "")
    )

    tematicas_insertadas = insertar_tabla_tematicas(
        doc,
        tabla_tematicas
    )
    ...
    diagnostico = {
        ...
        "competencias_programa_desde_matriz": len(
            datos_matriz.get("competencias_programa", [])
        ),
        "rdap_desde_matriz": len(
            datos_matriz.get("rdap_programa", [])
        ),
        "secciones_pdf": {
            k: bool((v or "").strip())
        ...
        "nota_tematicas": (
            "Tabla final Semana | Sesión | Contenido, sin columna Fecha. "
            "Se omiten festivos y recesos."
        ),
